...
export default function GradesPage() {
  ...
  // Fetch grades with pagination and search
  useEffect(() => {
    let requestData = {
      page: currentPage,
      limit: itemsPerPage,
  ...
  // Generate page numbers for pagination (similar to ClassPage)
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-[36px] border border-[#E2E7E4] bg-white px-6 py-6 shadow-[0_40px_80px_-60px_rgba(11,75,49,0.45)] sm:px-10">
        ...
        {/* Pagination - Updated to match ClassPage style */}
        {pagination && pagination.totalCount > 0 && (
          <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-[#8A928F]">
              Showing {grades.length} of {pagination.totalCount} entries
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className={`rounded-full border border-[#C5D2CD] bg-white px-3 py-2 text-sm text-[#0B4B31] transition ${
                    currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#F3F6F5]'
                  }`}
                >
                  ‹
                </button>
                ...
                <button 
                  ...
                  className={`rounded-full border border-[#C5D2CD] bg-white px-3 py-2 text-sm text-[#0B4B31] transition ${
                    currentPage === pagination.totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#F3F6F5]'
                  }`}
                >
                  ›
                </button>
