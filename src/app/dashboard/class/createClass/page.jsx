...
const Page = () => {
  ...
  useEffect(() => {
    if (showSuccess && (formData.name || formData.subject || formData.code || formData.description)) {
      setShowSuccess(false);
    }
  }, [formData.name, formData.subject, formData.code, formData.description, showSuccess]);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col p-4 sm:p-6 md:p-8">
      ...
      <div className="bg-white shadow-md rounded-2xl p-6 sm:p-8 w-full max-w-7xl">
        ...
        {loading && (
          <div className="bg-blue-100 border border-blue-400 text-blue-700 px-4 py-3 rounded-full text-center mb-6 animate-pulse">
            Creating class...
          </div>
        )}

        {showSuccess && (
          <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-full text-center mb-6 animate-fade-in">
            ...
            <button
              ...
              className={`rounded-full px-8 py-3 text-sm font-semibold transition ${
                loading || fetchingTeachers
                  ...
                  : "bg-[#E5EFEB] text-[#0B4B31] hover:bg-[#D4E6DE] hover:scale-105 active:scale-95"
              }`}
            >
