    ...
    <div className="space-y-8">
      ...
      {error && invoices.length === 0 && (
        <div className="rounded-[36px] border border-red-300 bg-red-50 px-6 py-6 text-red-700">
          ...
          <button
            onClick={() => dispatch(getAllAdminInvoices())}
      ...
      <section className="rounded-[36px] border border-[#E2E7E4] bg-white px-6 py-6 shadow-[0_40px_80px_-60px_rgba(11,75,49,0.45)] sm:px-10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-lg font-semibold text-[#104D2E]">Admin Invoices</h2>

          <div className="flex flex-wrap items-center gap-3">
            {/* Add New Invoice Button - Only for Super Admin */}
            {mainText === "Super Admin" && (
              <button
                type="button"
                onClick={() => router.push("/dashboard/institute-finance/invoice/add")}
                className="rounded-full bg-[#0B4B31] px-6 py-3 text-sm font-normal text-white transition hover:bg-[#0a3d25]"
              >
                + Add New Invoice
              </button>
            )}
          ...
          <table className="min-w-full border-separate border-spacing-y-3 text-left text-sm text-[#333]">
            <thead className="text-xs font-semibold uppercase tracking-wide text-[#8A928F]">
              <tr>
                ...
                <th className="px-4 font-normal text-[#0000008C]">Total Amount</th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span>👤</span>
                        <span className="font-medium text-[#1e1e1e]">{invoice.adminName}</span>
                      </div>
                    </td>
