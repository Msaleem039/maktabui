...
export default function AssignmentPage() {
  ...
  const [selectedAssignmentForRemarks, setSelectedAssignmentForRemarks] =
    useState(null);
  ...
  const {
    ...
    status,
    error,
    uploadSolutionStatus,
    totalCount,
    deleteStatus,
  } = useSelector((state) => state.assignment);
  const { createStatus: gradeCreateStatus } = useSelector(
    (state) => state.grade
  );
  ...
  const getActionMenuItems = () => {
    const baseItems = [{ label: "View Detail", icon: Eye, action: "view" }];
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      alert(
        `Failed to delete assignment: ${error.message || "Please try again."}`
      );
    }
  };
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        alert(
          "Please upload a valid file type (PDF, DOC, DOCX, TXT, JPG, PNG)"
        );
        return;
      }
  ...
  const handleSubmitSolution = async () => {
    ...
    try {
      ...
      alert(
        `Failed to upload solution: ${error.message || "Please try again."}`
      );
    } finally {
  ...
  const getStatusBadge = (dueDate) => {
    if (!dueDate)
      return { text: "No Due Date", style: "bg-gray-100 text-gray-600" };
          ...
          {openDropdownId === assignment._id && (
            <div
              ...
              className={`absolute right-0 z-50 min-w-[240px] rounded-2xl border border-[#D2E2DB] bg-white shadow-[0_18px_45px_-18px_rgba(11,75,49,0.35)] overflow-hidden ${
                dropdownDirections[assignment._id] === "up"
                  ? "bottom-full mb-3"
                  : "top-full mt-3"
              }`}
            >
              {actionMenuItems.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={(e) =>
                      handleActionClick(item.action, assignment._id, e)
                    }
                    className={`w-full flex items-center gap-3 px-5 py-3 text-sm font-semibold text-[#0B4B31] transition-all duration-150 ${
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-[36px] border border-[#E2E7E4] bg-white px-6 py-6 shadow-[0_40px_80px_-60px_rgba(11,75,49,0.45)] sm:px-10">
        ...
        <div className="mt-6">
          <div className="overflow-x-auto overflow-y-auto max-h-[500px] rounded-xl [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <table className="min-w-[1200px] w-full border-separate border-spacing-y-3 text-left text-sm text-[#333]">
              ...
              <thead className="sticky top-0 bg-white z-10 text-xs font-semibold uppercase tracking-wide text-[#8A928F]">
                <tr>
                  <th className="px-4 py-2 font-normal text-[#0000008C]">
                    Title
                  </th>
                  <th className="px-4 py-2 font-normal text-[#0000008C]">
                    Due Date
                  ...
                  <th className="px-4 py-2 text-right font-normal text-[#0000008C]">
                    Actions
                  </th>
                </tr>
              </thead>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-10">
          <div className="relative w-full max-w-xl rounded-[28px] bg-white px-6 py-7 shadow-[0_40px_120px_rgba(0,0,0,0.25)]">
            ...
            <div className="space-y-6">
              <div>
                <h3 className="text-2xl font-semibold text-[#0B4B31]">
                  Upload
                </h3>
                ...
                <div className="rounded-2xl bg-[#F8FAF9] p-5">
                  ...
                  <div className="space-y-1 text-sm text-gray-600">
                    ...
                    <p>
                      Student:{" "}
                      <span className="text-gray-800">
                        {selectedAssignment.student?.studentName || "—"}
                      </span>
                    </p>
                    <p>
                      Student ID:{" "}
                      <span className="text-gray-800">
                        {selectedAssignment.student?._id || "—"}
                      </span>
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <label
                  ...
                  className={`flex w-full cursor-pointer items-center justify-between rounded-2xl border border-[#0B4B31]/20 px-4 py-3 text-sm text-[#0B4B31] shadow-sm transition ${
                    isUploading
                ...
                >
                  <span className="truncate pr-4">
                    {uploadFile
                      ? uploadFile.name
                      : "Choose file (PDF, DOC, DOCX, TXT, JPG, PNG)"}
                  </span>
            ...
            {selectedAssignmentForRemarks && (
              <div className="mb-6 rounded-2xl bg-[#F4F7F5] p-4 text-sm text-[#1E1E1E] space-y-1">
                ...
                <p>
                  ...
                  {selectedAssignmentForRemarks.subject || "N/A"}
                </p>
                <p>
                  <span className="font-medium">Total Marks:</span>{" "}
                  {selectedAssignmentForRemarks.totalMarks || "N/A"}
                </p>
                <p>
                  <span className="font-medium">Student:</span>{" "}
                  {selectedAssignmentForRemarks.student?.studentName || "N/A"}
                </p>
                <p>
                  <span className="font-medium">Teacher:</span>{" "}
                  {selectedAssignmentForRemarks.teacher?.fullName || "N/A"}
                </p>
                <p className="truncate">
                  <span className="font-medium">Teacher ID:</span>{" "}
                  {selectedAssignmentForRemarks.teacher?._id || "N/A"}
                </p>
              </div>
            )}
                ...
                <div>
                  ...
                  {selectedAssignmentForRemarks?.totalMarks && (
                    <p className="mt-1 text-xs text-[#5E6C64]">
                      Out of {selectedAssignmentForRemarks.totalMarks} total
                      marks
                    </p>
                  )}
                </div>
