  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`absolute bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md rounded-b-[28px] p-4 border-t border-white/20 transition-all duration-300 ${
            isExpanded ? "relative rounded-t-[28px] mt-24" : ""
          }`}
        >
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <button
              onClick={onToggle}
              className={`w-8 h-8 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center transition-all duration-300 hover:bg-white hover:scale-110 ${
                isHovered || isExpanded ? "shadow-lg" : "shadow-md"
              }`}
            >
              {isExpanded ? (
          ...
          {/* Expanded Details */}
          {isExpanded && (
            <div className="mt-4 pt-5 border-t border-gray-200/60 space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
